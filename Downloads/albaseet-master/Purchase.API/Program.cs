...
using Purchases.Service.Extensions;
...
var purchasesAPI = configuration.GetValue<string>("Application:PurchasesAPI");
...
builder.Services.AddCors(o => o.AddPolicy("MyPolicy", x => { x.AllowAnyOrigin().AllowAnyMethod().AllowAnyHeader(); }));
...
//AutoMapper
var mappingConfig = new MapperConfiguration(mc => { mc.AddProfile(new PurchasesMappingProfile()); });
...
if (allowSwagger)
{
	...
	app.UseSwaggerUI(c =>
	{
		c.SwaggerEndpoint($"{purchasesAPI}/swagger/v1/swagger.json", "Purchases APIs");
		...
		c.DocumentTitle = "Purchases APIs";
	});
}
