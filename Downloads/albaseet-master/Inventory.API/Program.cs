...
var inventoryAPI = configuration.GetValue<string>("Application:InventoryAPI");
...
//AutoMapper
var mappingConfig = new MapperConfiguration(mc => { mc.AddProfile(new InventoryMappingProfile()); });
...
if (allowSwagger)
{
	...
	app.UseSwaggerUI(c =>
	{
		c.SwaggerEndpoint($"{inventoryAPI}/swagger/v1/swagger.json", "Inventory APIs");
		...
		c.DocumentTitle = "Inventory APIs";
	});
}
