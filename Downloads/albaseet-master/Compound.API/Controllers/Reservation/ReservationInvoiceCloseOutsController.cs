...
namespace Sales.API.Controllers
{
    ...
    [ApiController]
    [Authorize]
    public class ReservationInvoiceCloseOutsController : ControllerBase
    {
        ...
        public ReservationInvoiceCloseOutsController(IDatabaseTransaction databaseTransaction, IStringLocalizer<HandelException> exLocalizer, IHandleApprovalRequestService handleApprovalRequestService, IApprovalSystemService approvalSystemService, ISalesInvoiceReturnHeaderService salesInvoiceReturnHeaderService, ISalesInvoiceReturnService salesInvoiceReturnService, ISalesInvoiceReturnDetailService salesInvoiceReturnDetailService, ISalesInvoiceReturnHandlingService salesInvoiceReturnHandlingService, IGenericMessageService genericMessageService, IReservationInvoiceCloseOutService reservationInvoiceCloseOutService, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<IActionResult> ReadSalesInvoiceReturns(DataSourceLoadOptions loadOptions)
        {
            try
            {
                var data = await DataSourceLoader.LoadAsync(_salesInvoiceReturnHeaderService.GetUserSalesInvoiceReturnHeaders(MenuCodeData.ReservationInvoiceCloseOut), loadOptions);
                return Ok(data);
            }
            catch
            {
                var model = _salesInvoiceReturnHeaderService.GetUserSalesInvoiceReturnHeaders(MenuCodeData.ReservationInvoiceCloseOut);
        ...
        public async Task<IActionResult> ReadSalesInvoiceReturnsByStoreId(DataSourceLoadOptions loadOptions, int storeId, int clientId, int salesInvoiceReturnHeaderId)
        {
            try
            {
        ...
        public async Task<IActionResult> GetSalesInvoiceReturnCode(int storeId, DateTime documentDate, bool creditPayment)
        {
            var data = await _salesInvoiceReturnHeaderService.GetSalesInvoiceReturnCode(storeId, documentDate, true, false, creditPayment);
            return Ok(data);
        }

        [HttpGet]
        [Route("GetSalesInvoiceReturnFromSalesInvoice")]
        public async Task<IActionResult> GetSalesInvoiceReturnFromSalesInvoice(int salesInvoiceHeaderId)
        {
            var data = await _salesInvoiceReturnHandlingService.GetSalesInvoiceReturnFromSalesInvoice(salesInvoiceHeaderId, false, true);
            return Ok(data);
        }
        ...
        public async Task<IActionResult> Save([FromBody] SalesInvoiceReturnDto model, int requestId)
        {
            ...
            try
            {
                ...
                {
                    var hasApprove = await _approvalSystemService.IsMenuHasApprove(MenuCodeData.ReservationInvoiceCloseOut);
        ...
        private async Task<ResponseDto> SaveSalesInvoiceReturn(SalesInvoiceReturnDto model, bool hasApprove, bool approved, int? requestId)
        {
            await _databaseTransaction.BeginTransaction();
            var response = await _reservationInvoiceCloseOutService.SaveReservationInvoiceCloseOut(model, hasApprove, approved, requestId);
        ...
        public async Task<IActionResult> DeleteSalesInvoiceReturn(int salesInvoiceReturnHeaderId, int requestId)
        {
            ...
            try
            {
                ...
                if (model.SalesInvoiceReturnHeader != null)
                {
                    var menuCode = MenuCodeData.ReservationInvoiceCloseOut;
                    ...
                    {
                        await _databaseTransaction.BeginTransaction();
                        response = await _reservationInvoiceCloseOutService.DeleteReservationInvoiceCloseOut(salesInvoiceReturnHeaderId, menuCode);
						if (response.Success)
						{
							await _databaseTransaction.Commit();
						}
        ...
        private async Task<ResponseDto> SendApproveRequest([FromQuery] SalesInvoiceReturnDto? newModel, [FromQuery] SalesInvoiceReturnDto? oldModel, int requestId, int salesInvoiceReturnHeaderId, bool isDelete)
        {
