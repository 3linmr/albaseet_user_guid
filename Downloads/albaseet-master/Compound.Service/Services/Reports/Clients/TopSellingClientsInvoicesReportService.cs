...
{
    public class TopSellingClientsInvoicesReportService : ITopSellingClientsInvoicesReportService
    {
        ...
        public async Task<IQueryable<TopSellingClientsInvoicesReportDto>> GetTopSellingClientsInvoicesReport(int companyId, DateTime? fromDate, DateTime? toDate)
        {
            ...
            return from client in _clientService.GetAllClients().Where(x => x.CompanyId == companyId)
                   ...
                   from sales in _totalClientSales.GetTotalClientSales(fromDate, toDate, false).Where(x => x.ClientId == client.ClientId).DefaultIfEmpty()
                   orderby ((sales.InvoiceCount != 0 && sales.InvoiceCount != null) ? sales.InvoiceCount : 0) descending
                   select new TopSellingClientsInvoicesReportDto
                   {
