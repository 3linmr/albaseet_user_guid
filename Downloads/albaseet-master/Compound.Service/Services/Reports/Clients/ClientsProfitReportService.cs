...
{
	public class ClientsProfitReportService: IClientsProfitReportService
	{
		...
		public ClientsProfitReportService(IHttpContextAccessor httpContextAccessor, ISalesInvoiceHeaderService salesInvoiceHeaderService, ISalesInvoiceReturnHeaderService salesInvoiceReturnHeaderService, IClientCreditMemoService clientCreditMemoService, IClientDebitMemoService clientDebitMemoService, IStoreService storeService, IBranchService branchService, IClientService clientService, ITotalClientSalesService totalClientSalesService, IClientBalanceService clientBalanceService)
		{
			...
			_totalClientSalesService = totalClientSalesService;
			_clientBalanceService = clientBalanceService;
		}

		public async Task<IQueryable<ClientsProfitReportDto>> GetClientsProfitReport(int companyId, DateTime? fromDate, DateTime? toDate)
		{
			var language = _httpContextAccessor.GetProgramCurrentLanguage();

			return from client in _clientService.GetAllClients().Where(x => x.CompanyId == companyId)
				   from sales in _totalClientSalesService.GetTotalClientSales(fromDate, toDate, false).Where(x => x.ClientId == client.ClientId).DefaultIfEmpty()
				   ...
				   select new ClientsProfitReportDto
				   {
