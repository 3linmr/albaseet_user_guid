...
{
	public class ReservationInvoiceFollowUpDetailReportService : IReservationInvoiceFollowUpDetailReportService
	{
		...
		public async Task<IQueryable<ReservationInvoiceFollowUpDetailReportDto>> GetReservationInvoiceFollowUpDetailReport(List<int> storeIds, DateTime? fromDate, DateTime? toDate)
		{
			var language = _httpContextAccessor.GetProgramCurrentLanguage();


			var stockOutFromReservations = await (from reservationInvoiceHeader in _salesInvoiceHeaderService.GetAll().Where(x => storeIds.Contains(x.StoreId) && (fromDate == null || x.DocumentDate >= fromDate) && (toDate == null || x.DocumentDate <= toDate) && x.IsOnTheWay)
												  from stockOutFromSalesInvoiceHeader in _stockOutHeaderService.GetAll().Where(x => x.SalesInvoiceHeaderId == reservationInvoiceHeader.SalesInvoiceHeaderId)
												  from stockOutDetail in _stockOutDetailService.GetAll().Where(x => x.StockOutHeaderId == stockOutFromSalesInvoiceHeader.StockOutHeaderId)
												  group new { stockOutFromSalesInvoiceHeader, stockOutDetail } by new { reservationInvoiceHeader.SalesInvoiceHeaderId, stockOutDetail.ItemId, stockOutDetail.ItemPackageId } into g
												  select new { g.Key.SalesInvoiceHeaderId, g.Key.ItemId, g.Key.ItemPackageId, Quantity = g.Sum(x => x.stockOutDetail.Quantity + x.stockOutDetail.BonusQuantity), DocumentFullCodes = string.Join(", ", g.Select(x => x.stockOutFromSalesInvoiceHeader).Distinct().Select(x => x.Prefix + x.DocumentCode + x.Suffix)) })
												  .ToDictionaryAsync(x => new { x.SalesInvoiceHeaderId, x.ItemId, x.ItemPackageId }, x => new { x.Quantity, x.DocumentFullCodes });

			var stockOutReturnFromInvoicedStockOuts = await (from reservationInvoiceHeader in _salesInvoiceHeaderService.GetAll().Where(x => storeIds.Contains(x.StoreId) && (fromDate == null || x.DocumentDate >= fromDate) && (toDate == null || x.DocumentDate <= toDate) && x.IsOnTheWay)
															 from stockOutFromSalesInvoiceHeader in _stockOutHeaderService.GetAll().Where(x => x.SalesInvoiceHeaderId == reservationInvoiceHeader.SalesInvoiceHeaderId)
															 from stockOutReturnFromInvoicedStockOutHeader in _stockOutReturnHeaderService.GetAll().Where(x => x.StockOutHeaderId == stockOutFromSalesInvoiceHeader.StockOutHeaderId)
															 from stockOutReturnDetail in _stockOutReturnDetailService.GetAll().Where(x => x.StockOutReturnHeaderId == stockOutReturnFromInvoicedStockOutHeader.StockOutReturnHeaderId)
															 group new { stockOutReturnFromInvoicedStockOutHeader, stockOutReturnDetail } by new { reservationInvoiceHeader.SalesInvoiceHeaderId, stockOutReturnDetail.ItemId, stockOutReturnDetail.ItemPackageId } into g
															 select new { g.Key.SalesInvoiceHeaderId, g.Key.ItemId, g.Key.ItemPackageId, Quantity = g.Sum(x => x.stockOutReturnDetail.Quantity + x.stockOutReturnDetail.BonusQuantity), DocumentFullCodes = string.Join(", ", g.Select(x => x.stockOutReturnFromInvoicedStockOutHeader).Distinct().Select(x => x.Prefix + x.DocumentCode + x.Suffix)) })
														     .ToDictionaryAsync(x => new { x.SalesInvoiceHeaderId, x.ItemId, x.ItemPackageId }, x => new { x.Quantity, x.DocumentFullCodes });

			var reservationInvoiceCloseOuts = await (from reservationInvoiceHeader in _salesInvoiceHeaderService.GetAll().Where(x => storeIds.Contains(x.StoreId) && (fromDate == null || x.DocumentDate >= fromDate) && (toDate == null || x.DocumentDate <= toDate) && x.IsOnTheWay)
													 from salesInvoiceReturnHeader in _salesInvoiceReturnHeaderService.GetAll().Where(x => x.SalesInvoiceHeaderId == reservationInvoiceHeader.SalesInvoiceHeaderId)
													 ...
													 where salesInvoiceReturnHeader.IsOnTheWay
													 group new { salesInvoiceReturnHeader, salesInvoiceReturnDetail } by new { reservationInvoiceHeader.SalesInvoiceHeaderId, salesInvoiceReturnDetail.ItemId, salesInvoiceReturnDetail.ItemPackageId } into g
			...
			var stockOutReturnFromSalesInvoices = await (from reservationInvoiceHeader in _salesInvoiceHeaderService.GetAll().Where(x => storeIds.Contains(x.StoreId) && (fromDate == null || x.DocumentDate >= fromDate) && (toDate == null || x.DocumentDate <= toDate) && x.IsOnTheWay)
														 from stockOutReturnFromSalesInvoiceHeader in _stockOutReturnHeaderService.GetAll().Where(x => x.SalesInvoiceHeaderId == reservationInvoiceHeader.SalesInvoiceHeaderId)
														 ...
														 group new { stockOutReturnFromSalesInvoiceHeader, stockOutReturnDetail } by new { reservationInvoiceHeader.SalesInvoiceHeaderId, stockOutReturnDetail.ItemId, stockOutReturnDetail.ItemPackageId } into g
			...
			var salesInvoiceReturns = await (from reservationInvoiceHeader in _salesInvoiceHeaderService.GetAll().Where(x => storeIds.Contains(x.StoreId) && (fromDate == null || x.DocumentDate >= fromDate) && (toDate == null || x.DocumentDate <= toDate) && x.IsOnTheWay)
											 from salesInvoiceReturnHeader in _salesInvoiceReturnHeaderService.GetAll().Where(x => x.SalesInvoiceHeaderId == reservationInvoiceHeader.SalesInvoiceHeaderId)
											 from salesInvoiceReturnDetail in _salesInvoiceReturnDetailService.GetAll().Where(x => x.SalesInvoiceReturnHeaderId == salesInvoiceReturnHeader.SalesInvoiceReturnHeaderId)
											 where !salesInvoiceReturnHeader.IsOnTheWay
											 group new { salesInvoiceReturnHeader, salesInvoiceReturnDetail } by new { reservationInvoiceHeader.SalesInvoiceHeaderId, salesInvoiceReturnDetail.ItemId, salesInvoiceReturnDetail.ItemPackageId } into g
			...
			var query = from salesInvoiceHeader in _salesInvoiceHeaderService.GetAll().Where(x => storeIds.Contains(x.StoreId) && (fromDate == null || x.DocumentDate >= fromDate) && (toDate == null || x.DocumentDate <= toDate))
						...
						where salesInvoiceHeader.IsOnTheWay
						orderby salesInvoiceHeader.DocumentDate, salesInvoiceHeader.SalesInvoiceHeaderId
						select new ReservationInvoiceFollowUpDetailReportDto
						{
							...
							StockOutFromReservationQuantity = stockOutFromReservations.GetValueOrDefault(new { salesInvoiceHeader.SalesInvoiceHeaderId, salesInvoiceDetail.ItemId, salesInvoiceDetail.ItemPackageId }, anonymousDefault).Quantity,
							StockOutFromReservationDocumentFullCodes = stockOutFromReservations.GetValueOrDefault(new { salesInvoiceHeader.SalesInvoiceHeaderId, salesInvoiceDetail.ItemId, salesInvoiceDetail.ItemPackageId }, anonymousDefault).DocumentFullCodes,
							StockOutReturnFromInvoicedStockOutQuantity = stockOutReturnFromInvoicedStockOuts.GetValueOrDefault(new { salesInvoiceHeader.SalesInvoiceHeaderId, salesInvoiceDetail.ItemId, salesInvoiceDetail.ItemPackageId }, anonymousDefault).Quantity,
							StockOutReturnFromInvoicedStockOutDocumentFullCodes = stockOutReturnFromInvoicedStockOuts.GetValueOrDefault(new { salesInvoiceHeader.SalesInvoiceHeaderId, salesInvoiceDetail.ItemId, salesInvoiceDetail.ItemPackageId }, anonymousDefault).DocumentFullCodes,
							ReservationInvoiceCloseOutQuantity = reservationInvoiceCloseOuts.GetValueOrDefault(new { salesInvoiceHeader.SalesInvoiceHeaderId, salesInvoiceDetail.ItemId, salesInvoiceDetail.ItemPackageId }, anonymousDefault).Quantity,
							ReservationInvoiceCloseOutDocumentFullCodes = reservationInvoiceCloseOuts.GetValueOrDefault(new { salesInvoiceHeader.SalesInvoiceHeaderId, salesInvoiceDetail.ItemId, salesInvoiceDetail.ItemPackageId }, anonymousDefault).DocumentFullCodes,
