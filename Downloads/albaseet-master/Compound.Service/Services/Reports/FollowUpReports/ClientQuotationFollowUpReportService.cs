...
{
	public class ClientQuotationFollowUpReportService : IClientQuotationFollowUpReportService
	{
		...
		public IQueryable<ClientQuotationFollowUpReportDto> GetClientQuotationFollowUpReport(List<int> storeIds, DateTime? fromDate, DateTime? toDate)
		{
			...
			var query = from clientQuotationHeader in _clientQuotationHeaderService.GetAll().Where(x => storeIds.Contains(x.StoreId) && (fromDate == null || x.DocumentDate >= fromDate) && (toDate == null || x.DocumentDate <= toDate))
						...
						group new { clientQuotationHeader, clientQuotationApprovalHeader, proformaInvoiceHeader, stockOutFromProformaInvoice, stockOutReturnFromStockOut, salesInvoiceHeader, stockOutReturnFromSalesInvoice, salesInvoiceReturnHeader, clientDebitMemo, clientCreditMemo, store, menu }
						by new { clientQuotationHeader.ClientQuotationHeaderId, clientQuotationHeader.ClientId, client.ClientNameAr, client.ClientNameEn, menu.MenuCode, menu.MenuNameAr, menu.MenuNameEn, clientQuotationHeader.Prefix, clientQuotationHeader.DocumentCode, clientQuotationHeader.Suffix, clientQuotationHeader.DocumentDate, clientQuotationHeader.NetValue, clientQuotationHeader.Reference, clientQuotationHeader.RemarksAr, clientQuotationHeader.RemarksEn, store.StoreId, store.StoreNameAr, store.StoreNameEn, clientQuotationHeader.CreatedAt, clientQuotationHeader.UserNameCreated, clientQuotationHeader.ModifiedAt, clientQuotationHeader.UserNameModified }
						into g
						orderby g.Key.DocumentDate, g.Key.ClientQuotationHeaderId
						select new ClientQuotationFollowUpReportDto
						{
							ClientQuotationHeaderId = g.Key.ClientQuotationHeaderId,
