...
{
	public class ReservationInvoiceFollowUpReportService : IReservationInvoiceFollowUpReportService
	{
		...
		public IQueryable<ReservationInvoiceFollowUpReportDto> GetReservationInvoiceFollowUpReport(List<int> storeIds, DateTime? fromDate, DateTime? toDate)
		{
			...
			var query = from salesInvoiceHeader in _salesInvoiceHeaderService.GetAll().Where(x => storeIds.Contains(x.StoreId) && (fromDate == null || x.DocumentDate >= fromDate) && (toDate == null || x.DocumentDate <= toDate))
						...
						from stockOutFromReservationInvoice in _stockOutHeaderService.GetAll().Where(x => x.SalesInvoiceHeaderId == salesInvoiceHeader.SalesInvoiceHeaderId).DefaultIfEmpty()
						from stockOutReturnFromInvoicedStockOut in _stockOutReturnHeaderService.GetAll().Where(x => x.StockOutHeaderId == stockOutFromReservationInvoice.StockOutHeaderId).DefaultIfEmpty()
						...
						group new { salesInvoiceHeader, stockOutFromReservationInvoice, stockOutReturnFromInvoicedStockOut, stockOutReturnFromSalesInvoice, salesInvoiceReturnHeader, clientDebitMemo, clientCreditMemo, store, menu }
						by new { salesInvoiceHeader.SalesInvoiceHeaderId, salesInvoiceHeader.ClientId, client.ClientNameAr, client.ClientNameEn, menu.MenuCode, menu.MenuNameAr, menu.MenuNameEn, salesInvoiceHeader.Prefix, salesInvoiceHeader.DocumentCode, salesInvoiceHeader.Suffix, salesInvoiceHeader.DocumentDate, salesInvoiceHeader.NetValue, salesInvoiceHeader.Reference, salesInvoiceHeader.RemarksAr, salesInvoiceHeader.RemarksEn, store.StoreId, store.StoreNameAr, store.StoreNameEn, salesInvoiceHeader.CreatedAt, salesInvoiceHeader.UserNameCreated, salesInvoiceHeader.ModifiedAt, salesInvoiceHeader.UserNameModified }
						into g
						orderby g.Key.DocumentDate, g.Key.SalesInvoiceHeaderId
						select new ReservationInvoiceFollowUpReportDto
						{
							SalesInvoiceHeaderId = g.Key.SalesInvoiceHeaderId,
							...
							StockOutFromReservation = g.Count(x => x.stockOutFromReservationInvoice != null) > 0 ? _localizer["Done"].ToString() : null,
							StockOutReturnFromInvoicedStockOut = g.Count(x => x.stockOutReturnFromInvoicedStockOut != null) > 0 ? _localizer["Done"].ToString() : null,
							ReservationInvoiceCloseOut = g.Count(x => x.salesInvoiceReturnHeader != null && x.salesInvoiceReturnHeader.IsOnTheWay) > 0 ? _localizer["Done"].ToString() : null,
							...
							SalesInvoiceReturn = g.Count(x => x.salesInvoiceReturnHeader != null && !x.salesInvoiceReturnHeader.IsOnTheWay) > 0 ? _localizer["Done"].ToString() : null,
