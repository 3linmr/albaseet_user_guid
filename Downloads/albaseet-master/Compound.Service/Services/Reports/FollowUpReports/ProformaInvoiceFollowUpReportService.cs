...
{
	public class ProformaInvoiceFollowUpReportService : IProformaInvoiceFollowUpReportService
	{
		...
		public ProformaInvoiceFollowUpReportService(IClientQuotationHeaderService clientQuotationHeaderService, IClientQuotationApprovalHeaderService clientQuotationApprovalHeaderService, IProformaInvoiceHeaderService proformaInvoiceHeaderService, IStockOutHeaderService stockOutHeaderService, ISalesInvoiceHeaderService salesInvoiceHeaderService, IStockOutReturnHeaderService stockOutReturnHeaderService, IStoreService storeService, IHttpContextAccessor httpContextAccessor, IStringLocalizer<ProductRequestFollowUpReportService> localizer, IClientCreditMemoService clientCreditMemoService, IClientDebitMemoService clientDebitMemoService, ISalesInvoiceReturnHeaderService salesInvoiceReturnHeaderService, IMenuService menuService, IClientService clientService, IDocumentStatusService documentStatusService)
		{
		...
		public IQueryable<ProformaInvoiceFollowUpReportDto> GetProformaInvoiceFollowUpReport(List<int> storeIds, DateTime? fromDate, DateTime? toDate)
		{
			...
			var query = from proformaInvoiceHeader in _proformaInvoiceHeaderService.GetAll().Where(x => storeIds.Contains(x.StoreId) && (fromDate == null || x.DocumentDate >= fromDate) && (toDate == null || x.DocumentDate <= toDate))
						...
						where salesInvoiceHeader == null || !salesInvoiceHeader.IsDirectInvoice
						group new { proformaInvoiceHeader, stockOutFromProformaInvoice, stockOutReturnFromStockOut, salesInvoiceHeader, stockOutReturnFromSalesInvoice, salesInvoiceReturnHeader, clientDebitMemo, clientCreditMemo, store, menu }
						by new { proformaInvoiceHeader.ProformaInvoiceHeaderId, proformaInvoiceHeader.ClientId, documentStatus.DocumentStatusNameAr, documentStatus.DocumentStatusNameEn, client.ClientNameAr, client.ClientNameEn, menu.MenuCode, menu.MenuNameAr, menu.MenuNameEn, proformaInvoiceHeader.Prefix, proformaInvoiceHeader.DocumentCode, proformaInvoiceHeader.Suffix, proformaInvoiceHeader.DocumentDate, proformaInvoiceHeader.NetValue, proformaInvoiceHeader.Reference, proformaInvoiceHeader.RemarksAr, proformaInvoiceHeader.RemarksEn, store.StoreId, store.StoreNameAr, store.StoreNameEn, proformaInvoiceHeader.CreatedAt, proformaInvoiceHeader.UserNameCreated, proformaInvoiceHeader.ModifiedAt, proformaInvoiceHeader.UserNameModified }
						into g
						orderby g.Key.DocumentDate, g.Key.ProformaInvoiceHeaderId
						select new ProformaInvoiceFollowUpReportDto
						{
							ProformaInvoiceHeaderId = g.Key.ProformaInvoiceHeaderId,
							...
							StockOutFromProformaInvoice = g.Count(x => x.stockOutFromProformaInvoice != null) > 0 ? _localizer["Done"].ToString() : null,
							StockOutReturnFromStockOut = g.Count(x => x.stockOutReturnFromStockOut != null) > 0 ? _localizer["Done"].ToString() : null,
