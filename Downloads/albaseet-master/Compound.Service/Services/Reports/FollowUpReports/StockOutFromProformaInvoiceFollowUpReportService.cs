...
{
	public class StockOutFromProformaInvoiceFollowUpReportService : IStockOutFromProformaInvoiceFollowUpReportService
	{
		...
		public IQueryable<StockOutFromProformaInvoiceFollowUpReportDto> GetStockOutFromProformaInvoiceFollowUpReport(List<int> storeIds, DateTime? fromDate, DateTime? toDate)
		{
			...
			var query = from stockOutHeader in _stockOutHeaderService.GetAll().Where(x => storeIds.Contains(x.StoreId) && (fromDate == null || x.DocumentDate >= fromDate) && (toDate == null || x.DocumentDate <= toDate))
						...
						from stockOutReturnFromStockOut in _stockOutReturnHeaderService.GetAll().Where(x => x.StockOutHeaderId == stockOutHeader.StockOutHeaderId).DefaultIfEmpty()
						...
						group new { stockOutHeader, stockOutReturnFromStockOut, salesInvoiceHeader, stockOutReturnFromSalesInvoice, salesInvoiceReturnHeader, clientDebitMemo, clientCreditMemo, store, menu }
						by new { stockOutHeader.StockOutHeaderId, stockOutHeader.ClientId, client.ClientNameAr, client.ClientNameEn, menu.MenuCode, menu.MenuNameAr, menu.MenuNameEn, stockOutHeader.Prefix, stockOutHeader.DocumentCode, stockOutHeader.Suffix, stockOutHeader.DocumentDate, stockOutHeader.NetValue, stockOutHeader.Reference, stockOutHeader.RemarksAr, stockOutHeader.RemarksEn, store.StoreId, store.StoreNameAr, store.StoreNameEn, stockOutHeader.CreatedAt, stockOutHeader.UserNameCreated, stockOutHeader.ModifiedAt, stockOutHeader.UserNameModified }
						into g
						orderby g.Key.DocumentDate, g.Key.StockOutHeaderId
						select new StockOutFromProformaInvoiceFollowUpReportDto
						{
							StockOutHeaderId = g.Key.StockOutHeaderId,
