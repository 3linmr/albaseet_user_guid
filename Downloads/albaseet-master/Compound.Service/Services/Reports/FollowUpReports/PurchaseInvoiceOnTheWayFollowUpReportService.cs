...
{
    public class PurchaseInvoiceOnTheWayFollowUpReportService : IPurchaseInvoiceOnTheWayFollowUpReportService
    {
        ...
        public IQueryable<PurchaseInvoiceOnTheWayFollowUpReportDto> GetPurchaseInvoiceOnTheWayFollowUpReport(List<int> storeIds, DateTime? fromDate, DateTime? toDate)
        {
            ...
            var query = from purchaseInvoiceHeader in _purchaseInvoiceHeaderService.GetAll().Where(x => storeIds.Contains(x.StoreId) && (fromDate == null || x.DocumentDate >= fromDate) && (toDate == null || x.DocumentDate <= toDate))
                        ...
                        from stockInFromPurchaseInvoice in _stockInHeaderService.GetAll().Where(x => x.PurchaseInvoiceHeaderId == purchaseInvoiceHeader.PurchaseInvoiceHeaderId).DefaultIfEmpty()
                        from stockInReturnFromInvoicedStockIn in _stockInReturnHeaderService.GetAll().Where(x => x.StockInHeaderId == stockInFromPurchaseInvoice.StockInHeaderId).DefaultIfEmpty()
                        ...
                        group new { purchaseInvoiceHeader, stockInFromPurchaseInvoice, stockInReturnFromInvoicedStockIn, stockInReturnFromPurchaseInvoice, purchaseInvoiceReturnHeader, supplierDebitMemo, supplierCreditMemo, store, menu }
                        ...
                        select new PurchaseInvoiceOnTheWayFollowUpReportDto
						{
							...
							StockInFromPurchaseInvoice = g.Count(x => x.stockInFromPurchaseInvoice != null) > 0 ? _localizer["Done"].ToString() : null,
							StockInReturnFromInvoicedStockIn = g.Count(x => x.stockInReturnFromInvoicedStockIn != null) > 0 ? _localizer["Done"].ToString() : null,
							PurchaseInvoiceReturnOnTheWay = g.Count(x => x.purchaseInvoiceReturnHeader != null && x.purchaseInvoiceReturnHeader.IsOnTheWay) > 0 ? _localizer["Done"].ToString() : null,
							...
							PurchaseInvoiceReturn = g.Count(x => x.purchaseInvoiceReturnHeader != null && !x.purchaseInvoiceReturnHeader.IsOnTheWay) > 0 ? _localizer["Done"].ToString() : null,
