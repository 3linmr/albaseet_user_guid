...
{
	public class SalesInvoiceInterimFollowUpReportService : ISalesInvoiceInterimFollowUpReportService
	{
		...
		public IQueryable<SalesInvoiceInterimFollowUpReportDto> GetSalesInvoiceInterimFollowUpReport(List<int> storeIds, DateTime? fromDate, DateTime? toDate)
		{
			...
			var query = from salesInvoiceHeader in _salesInvoiceHeaderService.GetAll().Where(x => storeIds.Contains(x.StoreId) && (fromDate == null || x.DocumentDate >= fromDate) && (toDate == null || x.DocumentDate <= toDate))
						...
						group new { salesInvoiceHeader, stockOutReturnFromSalesInvoice, salesInvoiceReturnHeader, clientDebitMemo, clientCreditMemo, store, menu }
						...
						select new SalesInvoiceInterimFollowUpReportDto
						{
