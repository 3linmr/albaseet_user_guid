...
{
	public class InventoryValueReportService: IInventoryValueReportService
	{
		...
		public async Task<IQueryable<InventoryValueReportDto>> GetInventoryValueReport(List<int> storeIds, DateTime? fromDate, DateTime? toDate, int? itemCategoryId, int? itemSubCategoryId, int? itemSectionId, int? itemSubSectionId, int? mainItemId, DateTime? expireBefore)
		{
			...
			var report = from item in _itemService.GetAll().Where(x => companyIds.Contains(x.CompanyId) && x.ItemTypeId != ItemTypeData.Service &&
						 ...
						 select new InventoryValueReportDto
						 {
							 ItemId = item.ItemId,
		...
		private IQueryable<InventoryValueReportQuantityDto> GetReportQuantitiesCalculated(DateTime? fromDate, DateTime? toDate)
		{
			return from generalInventoryDocument in _generalInventoryDocumentService.GetGeneralInventoryDocuments()
				   ...
				   select new InventoryValueReportQuantityDto
				   {
