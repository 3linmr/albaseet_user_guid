...
namespace Shared.Repository
{
    public class Repository<T> : IDisposable, IRepository<T> where T : BaseObject
    {
        ...
        public Repository(ApplicationDbContext context, ApplicationSettingDto application)
        {
			//DatabaseInitializer.ChangeDbContext(application, context,null).WaitAsync(TimeSpan.FromMinutes(1));
        ...
        {
            return _entities.AsQueryable().Where(x => x.Hide != true);
        }
